...
if __name__ == '__main__':
    ...
    augmenter = ImageAugmenter(96, 96,
                                          hflip=False, vflip=False,
                                          scale_to_percent=1.5, scale_axis_equally=True,
                                          rotation_deg=0, shear_deg=0,
                                          translation_x_px=0, translation_y_px=0,
                                          channel_is_first_axis=False)
    folder = '/data/liubo/face/MS-Celeb_face/example/test'
    pic_list = os.listdir(folder)
    augment_index = 0
    data0 = []
    data1 = []
    for pic in pic_list:
        img = misc.imresize(misc.imread(os.path.join(folder, pic)), (96, 96, 3))
        data0.append(img)
    data0 = np.asarray(data0)
    pdb.set_trace()
    data0 = augmenter.augment_batch(data0)
    new_folder = '/data/liubo/face/MS-Celeb_face/example/test_augment'
    if not os.path.exists(new_folder):
        os.makedirs(new_folder)
    for index in range(len(data0)):
        misc.imsave('%s/%d.png' %(new_folder, index), data0[index])
