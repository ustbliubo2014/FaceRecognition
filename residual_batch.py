...
class ResidualBatch(object):
    def __init__(self, options, maxsize=20, batch_size=128, epoch_num=10):
        ...
        if model_type == 'new_shape':
            self.input_shape = (3, 78, 62) #每个图片的shape
            self.pic_shape = (78, 62, 3)
            self.model_file = os.path.join(self.model_folder, 'vgg_face.small_data.new_shape.rgb.residual.model')
            self.weight_file = os.path.join(self.model_folder,'vgg_face.small_data.new_shape.rgb.residual.weight')
        elif model_type == 'small_rgb':
            self.input_shape = (3, 50, 50)#每个图片的shape
            self.pic_shape = (50, 50, 3)
            self.model_file = os.path.join(self.model_folder, 'vgg_face.all_data.small.rgb.residual.model')
            self.weight_file = os.path.join(self.model_folder, 'vgg_face.all_data.small.rgb.residual.weight')
...
if __name__ == '__main__':
    ...
    parser.add_option("-m", "--model_type",
                        default='new_shape',  help="model type")
    (options, args) = parser.parse_args()
    residual = ResidualBatch(options)
    residual.read_train()
