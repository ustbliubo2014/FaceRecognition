...
@author: liubo
@software: PyCharm
@file: inception_resnet_v1_small_v2.py
@time: 2017/2/6 15:59
@contact: ustb_liubo@qq.com
@annotation: inception_resnet_v1_small_v2
"""
import sys
import logging
...
# Inception-Resnet-C
def block8(net, kernel_size=192, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    """Builds the 8x8 resnet block."""
    with tf.variable_scope(scope, 'Block8', [net], reuse=reuse):
        with tf.variable_scope('Branch_0'):
            tower_conv = slim.conv2d(net, kernel_size, 1, scope='Conv2d_1x1')
        with tf.variable_scope('Branch_1'):
            tower_conv1_0 = slim.conv2d(net, kernel_size, 1, scope='Conv2d_0a_1x1')
            tower_conv1_1 = slim.conv2d(tower_conv1_0, kernel_size, [1, 3],
                                        scope='Conv2d_0b_1x3')
            tower_conv1_2 = slim.conv2d(tower_conv1_1, kernel_size, [3, 1],
                                        scope='Conv2d_0c_3x1')
        mixed = tf.concat(3, [tower_conv, tower_conv1_2])
        up = slim.conv2d(mixed, net.get_shape()[3], 1, normalizer_fn=None,
                         activation_fn=None, scope='Conv2d_1x1')
...
def reduction_b(net, kernel_size=256):
    with tf.variable_scope('Branch_0'):
        tower_conv = slim.conv2d(net, kernel_size, 1, scope='Conv2d_0a_1x1')
        tower_conv_1 = slim.conv2d(tower_conv, kernel_size, 3, stride=2,
                                   padding='VALID', scope='Conv2d_1a_3x3')
    with tf.variable_scope('Branch_1'):
        tower_conv1 = slim.conv2d(net, kernel_size, 1, scope='Conv2d_0a_1x1')
        tower_conv1_1 = slim.conv2d(tower_conv1, kernel_size, 3, stride=2,
                                    padding='VALID', scope='Conv2d_1a_3x3')
    with tf.variable_scope('Branch_2'):
        tower_conv2 = slim.conv2d(net, kernel_size, 1, scope='Conv2d_0a_1x1')
        tower_conv2_1 = slim.conv2d(tower_conv2, kernel_size, 3,
                                    scope='Conv2d_0b_3x3')
        tower_conv2_2 = slim.conv2d(tower_conv2_1, kernel_size, 3, stride=2,
                                    padding='VALID', scope='Conv2d_1a_3x3')
