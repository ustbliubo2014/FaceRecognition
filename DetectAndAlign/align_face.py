...
model_dir = '/data/liubo/face/annotate_face_model/'
...
class AlignDlib:
    ...
    def align(self, imgDim, rgbImg, bb=None, pad=None, ts=None,
        ...
        if bb is None:
            if opencv_det:
                ...
                faces = face_cascade.detectMultiScale(rgbImg, 1.1, 2, minSize=(60, 60))
                dlib_rects = []
    ...
    def detect(self, imgDim, rgbImg, bb=None, pad=None, ts=None,
              landmarks=None, landmarkIndices=INNER_EYES_AND_BOTTOM_LIP,
              opencv_det=False, opencv_model=opencv_model,
              only_crop=False):
        r"""align(imgDim, rgbImg, bb=None, landmarks=None, landmarkIndices=INNER_EYES_AND_BOTTOM_LIP)
        ...
        if bb is None:
            ...
            if bb is None:
                ...
                left = int(max(0, bb.left() - bb.width()))
                top = int(max(0, bb.top() - bb.height()))
                right = int(min(rgbImg.shape[1], bb.right() + bb.width()))
                bottom = int(min(rgbImg.shape[0], bb.bottom() + bb.height()))
...
if __name__ == '__main__':
    ...
    # ts = 0.1
    # face_size = 96
    # python align_face.py $raw_data_path align $landmarks $align_data_path --ts $ts --size $face_size
    ...
    parser.add_argument('--opencv-model', type=str, help="Path to dlib's face predictor.",
                        default=opencv_model)
    ...
    parser.add_argument('--dlibFacePredictor', type=str, help="Path to dlib's face predictor.",
                        default=os.path.join(model_dir, "shape_predictor_68_face_landmarks.dat"))
