...
Example usage:
        ...
        ia = ImageAugme
        ter(img_width, img_height, hlip=True, rotation_deg=20,
                            translation_x_px=5)
...
if __name__ == '__main__':
    ...
    augmenter = ImageAugmenter(150, 150,
                                  hflip=True, vflip=False,
                                  scale_to_percent=1., scale_axis_equally=False,
                                  rotation_deg=0, shear_deg=15,
                                  translation_x_px=0, translation_y_px=0,
                                  channel_is_first_axis=False
    )
    folder = 'D:\data/face/faceScrub/FaceScrub/Anthony_Hopkins'
    pic_list = os.listdir(folder)
    pic_array = []
    for pic in pic_list:
        pic_array.append(misc.imresize(misc.imread(os.path.join(folder, pic)), (150, 150, 3)))
    pic_array = np.asarray(pic_array)
    pic_array = augmenter.augment_batch(pic_array)
    new_folder = 'D:\data/Anthony_Hopkins_augment'
    if not os.path.exists(new_folder):
        os.makedirs(new_folder)
    for index in range(len(pic_array)):
        misc.imsave('%s/%d.png' %(new_folder, index), pic_array[index])
